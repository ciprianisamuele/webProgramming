...
if (isset($_POST['user_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $last_activity = strtotime($row['last_activity']); // Converti la stringa datetime in un timestamp unix
        $current_time = time(); // Ottieni il timestamp unix corrente
    
        if ($current_time - $last_activity < 60*3) { // Controlla se sono passati più di 3 minuti dall'ultima attività
            echo "Attivo ora";
        }
        else{
            echo "Non attivo";
